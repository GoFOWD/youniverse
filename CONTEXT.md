...
> [!IMPORTANT]
> **핵심 로직 변경 (2025-11-27)**
> 단순 1:1 매핑에서 **가중치 합산 방식**으로 변경되었습니다. 이 로직은 결과 분포의 균형(각 20~25%)을 위해 설계되었으므로, **임의로 수정하거나 이전 로직으로 되돌리지 마십시오.**

### 1. 정규화 (Normalization)
각 성향 점수(E, P, C)를 -2 ~ +2 범위로 변환하여 프론트엔드 그래프 표시에 사용합니다.
```typescript
...
### 2. 최종 결과 산출 (Weighted Composite Score)

#### 🌊 대양 (Ocean) - 탐구/사색 중심
- **공식**: `Curiosity(3) + Positivity(2) + Energy(1)`
- **우선순위**: 호기심 > 긍정 > 에너지
- **분포 기준** (10,000회 시뮬레이션 기반 5분위):
  - **남극해** (≤ 17): 하위 20%
  - **북극해** (≤ 23): 20~40%
  - **대서양** (≤ 28): 40~60%
  - **인도양** (≤ 34): 60~80%
...
#### 🍂 계절 (Season) - 활동/에너지 중심
- **공식**: `Energy(3) + Positivity(2) + Curiosity(1)`
- **우선순위**: 에너지 > 긍정 > 호기심
- **분포 기준** (10,000회 시뮬레이션 기반 4분위):
  - **겨울** (≤ 16): 하위 25%
  - **가을** (≤ 25): 25~50%
  - **봄** (≤ 34): 50~75%
  - **여름** (35+): 상위 25%
...
### 3. 최종 코드
- 형식: `{Ocean}-{Season}` (예: `태평양-여름`)
- 총 20가지 조합 (5 Ocean × 4 Season)
