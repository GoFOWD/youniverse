...
const Layout: React.FC<LayoutProps> = ({ children, step, progress = 0, ocean, className = '', isTransitioning = false }) => {
  ...
  return (
    <div className={`relative h-[100dvh] w-full ${step === 'result' ? 'overflow-y-auto' : 'overflow-hidden'} text-white`}>
      ...
      <main className={`relative z-10 flex flex-col items-center ${step === 'result' ? 'justify-start min-h-full' : 'justify-center h-full'} px-4 py-4 sm:py-6 md:py-8 mx-auto max-w-md w-full ${className}`}>
        {children}
      </main>
    </div>
  );
